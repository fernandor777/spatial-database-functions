...
CREATE TABLE ProjCircle2D ( id integer, label varchar2(20), angleDegrees number, rgb varchar2(20), iRGB integer, geom mdsys.sdo_geometry );
SET FEEDBACK OFF
    INSERT INTO ProjCircle2D (id, label, angleDegrees, rgb, iRGB, geom)
      SELECT rownum,
             CHR(dbms_random.value(65,90)) || to_char(round(dbms_random.value(0,1000),0),'FM9999') as label,
             ROUND(dbms_random.value(0,359.9),1) as angleDegrees,
...
DROP   TABLE LocalPoint2D PURGE;
CREATE TABLE LocalPoint2D ( geom MDSYS.SDO_GEOMETRY );
INSERT INTO  LocalPoint2D 
VALUES( MDSYS.SDO_GEOMETRY(2001, NULL, MDSYS.SDO_POINT_TYPE(10,20,NULL),NULL,NULL));
COMMIT;

DROP   TABLE LocalLine2D PURGE;
CREATE TABLE LocalLine2D ( linetype varchar2(40), geom mdsys.sdo_geometry );
INSERT INTO  LocalLine2D 
...
DROP   TABLE LocalPoly2D PURGE;
CREATE TABLE LocalPoly2D ( polytype varchar2(40), geom mdsys.sdo_geometry );
INSERT INTO  LocalPoly2D 
 VALUES ('VERTEXWITHHOLE',mdsys.sdo_geometry(2003,NULL,NULL,
...
DROP   TABLE Proj41914Poly2D PURGE;
CREATE TABLE Proj41914Poly2D ( geom mdsys.sdo_geometry );
INSERT INTO proj41914Poly2D 
VALUES ( MDSYS.SDO_GEOMETRY(2003, 41014, NULL, MDSYS.SDO_ELEM_INFO_ARRAY(1, 1003, 1),
         MDSYS.SDO_ORDINATE_ARRAY(
...
BEGIN
  ...
  INSERT INTO USER_SDO_GEOM_METADATA(TABLE_NAME,COLUMN_NAME,DIMINFO,SRID)
    ...
    MDSYS.SDO_DIM_ELEMENT('Longitude',V_SHAPE.SDO_ORDINATES(1),V_SHAPE.SDO_ORDINATES(2),0.05),
    MDSYS.SDO_DIM_ELEMENT('Latitude' ,V_SHAPE.SDO_ORDINATES(3),V_SHAPE.SDO_ORDINATES(4),0.05)),8311);
END;
/
SHOW ERRORS
...
BEGIN
  ...
  INSERT INTO USER_SDO_GEOM_METADATA(TABLE_NAME,COLUMN_NAME,DIMINFO,SRID)
    ...
    MDSYS.SDO_DIM_ELEMENT('Longitude',V_SHAPE.SDO_ORDINATES(1),V_SHAPE.SDO_ORDINATES(2),0.05),
    MDSYS.SDO_DIM_ELEMENT('Latitude' ,V_SHAPE.SDO_ORDINATES(3),V_SHAPE.SDO_ORDINATES(4),0.05),
    MDSYS.SDO_DIM_ELEMENT('Z'        ,V_SHAPE.SDO_ORDINATES(3),V_SHAPE.SDO_ORDINATES(4),0.5)),8311);
END;
/
SHOW ERRORS
