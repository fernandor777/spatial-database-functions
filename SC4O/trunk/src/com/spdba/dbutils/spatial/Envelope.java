...
{
    private static final LOGGER LOGGER = new LOGGER("com.spdba.spatial.Envelope");
     ...
     */
    private void init(double _x1, double _y1, double _x2, double _y2)
    {
        if (_x1 < _x2) {
    ...
    public Envelope(JGeometry _geo) 
    {
        ...
        if ( _geo.isOrientedPoint() || _geo.isOrientedMultiPoint() ) {
            this.setMBR(SDO.getOrientedPointMBR(_geo));
        } else {
            ...
            if ( mbr.length < 4 || Double.isInfinite(mbr[0]) || Double.isNaN(mbr[0]) ) {
                this.setMBR(SDO.getOrdinatesMBR(_geo));
            }
            if ( mbr.length < 4 ) {
                return;
            }
            switch ( dims ) {
                case 4: init(mbr[0], mbr[1], mbr[2], mbr[3]); break;
                case 3: init(mbr[0], mbr[1], mbr[3], mbr[4]); break;
                case 2: 
    ...
    public static final int ZOOM_N = 4;
    public static final int ZOOM_UP = 5;
    public static final int ZOOM_S = 6;
    public static final int ZOOM_DOWN = 7;
    public static final int ZOOM_E = 8;
    public static final int ZOOM_RIGHT = 9;
    public static final int ZOOM_W = 10;
    public static final int ZOOM_LEFT = 11;
    public static final int ZOOM_NE = 12;
    public static final int ZOOM_SE = 13;
    public static final int ZOOM_NW = 14;
    public static final int ZOOM_SW = 15;

    public int zoomOrPan(int _direction,
                         int _percentage) 
    {
        if ( _percentage == 0 ) {
            return 0;
    ...
    **/
    ...
    public String toSdoGeometry(String _SRID,
        ...
        return "sdo_geometry(2003," + 
               (Strings.isEmpty(_SRID) ? "NULL" : _SRID) + 
     ...
     public String toString() {
        ...
        return this.isNull() 
            ? "NULL"
            : "(" + MathUtils.round(this.minX, this.decimalPlaces) + "," +
               MathUtils.round(this.minY, this.decimalPlaces) + "),(" + MathUtils.round(this.maxX, this.decimalPlaces) + "," + MathUtils.round(this.maxY, this.decimalPlaces) + 
              ")";
    }

    public String toXML() {
        //return String.format("<MBR><MinX>%s</MinX><MinY>%s</MinY><>%s</MaxX><MaxY>%s</>",
        //                     this.minX,this.minY,this.maxX,this.maxY);
        return this.isNull() 
            ? "NULL"
            : "<MBR>" +
              "<MinX>" + MathUtils.round(this.minX, this.decimalPlaces) + "</MinX>" +
              "<MinY>" +
               MathUtils.round(this.minY, this.decimalPlaces) + "</MinY>" +
              "<MaxX>" +
               MathUtils.round(this.maxX, this.decimalPlaces) + "</MaxX>" +
              "<MaxY>" +
               MathUtils.round(this.maxY, this.decimalPlaces) + "</MaxY>" +
              "</MBR>";
    }
