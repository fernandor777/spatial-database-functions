...
public class Coordinate implements Comparable, Cloneable, Serializable {
  ...
  /**
    ...
    public Coordinate(double x, double y) {
      this(x, y, NULL_ORDINATE,NULL_ORDINATE);
    }
  ...
  /**
   *  Sets this <code>Coordinate</code>s (x,y,z,m) values to that of <code>other</code>.
   *
  ...
  /**
     ...
     * By examining z and m ordinates for equality with NULL_ORDINATE
     * this function determines the coordinate dimension of the Coordinate from its values.
     * Assumes that z and m of NULL_ORDINATE is not same as those ordinates actually having NULL values in real Geomtry objects.
     * @return coordDim of 2, 3 or 4
     */
    public int getCoordinateDimension() {
      int coordDim = 2;
      if (this.z != NULL_ORDINATE) coordDim++;
      if (this.m != NULL_ORDINATE) coordDim++;
      return coordDim;
  }
  ...
  /**
   *  Returns <code>true</code> if <code>other</code> has the same values for x,y,z and m.
   *
   *@param  other  a <code>Coordinate</code> with which to do the 3D comparison.
   *@return        <code>true</code> if <code>other</code> is a <code>Coordinate</code>
   *      with the same values for x, y and z.
   */
  public boolean equals4D(Coordinate other)
  {
        return (x == other.x) && 
               (y == other.y) && 
               (z == other.z || (Double.isNaN(z) && Double.isNaN(other.z))) && 
  ...
  /**
   *  Returns a <code>String</code> of the form <I>(x y) or (x y z) or (x y z m)</I> .
   * @param _forceDimension Forces return of coordinate string to desired dimension.
   * @return    a <code>String</code> of the form <I>(x y) or (x y z) or (x y z m)</I>

   */
  public String toString(int _forceDimension) {
      String coordString = null;
      switch (_forceDimension) {
      case 2 : coordString = x + " " + y; break;
      case 3 : coordString = x + " " + y + " ";
               ...
               break;
      case 4  : 
      default : coordString = x + 
                             " " + 
                             y + 
                             " " +
                             (Double.isNaN(z) ? "NULL" : z) + 
                             " " +
      ...
      implements Comparator
  {
    ...
    public DimensionalComparator(int dimensionsToTest)
    {
      if (dimensionsToTest != 2 && 
        ...
        throw new IllegalArgumentException("only 2, 3 or 4 dimensions may be specified");
    ...
    public int compare(Object o1, Object o2)
    {
      ...
      if (dimensionsToTest <= 3) return 0;

      int compM = compare(c1.z, c2.z);
      return compM;
