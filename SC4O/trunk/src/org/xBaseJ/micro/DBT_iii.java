...
class DBT_iii extends DBTFile {

    boolean stopOnSizeLimit = false;
    int limitSize = 2048;

    public DBT_iii(DBF iDBF, boolean readOnly) throws IOException, xBaseJException {
        super(iDBF, readOnly, DBF.DBASEIII_WITH_MEMO);

        String sizeLimit = System.getProperty("xBaseJ_LimitMemoSizeToThisLength");
        if (sizeLimit == null)
            return;
        stopOnSizeLimit = true;
        try {
            limitSize = Integer.parseInt(sizeLimit);
        } catch (NumberFormatException nfe) {
            nfe.printStackTrace();
            throw new xBaseJException("xBaseJ_LimitMemoSizeToThisLength is in error.");
        }
    }
    ...
    public byte[] readBytes(byte[] input) throws IOException, xBaseJException {
        ...
        do {
            ...
            workLength += i;
            if (stopOnSizeLimit = true) {
                if (bTemp2.length > limitSize) {
                    work = false;
                }
            }
        } while (work);
    ...
    public byte[] write(String value, int originalSize, boolean write, byte originalPos[]) throws IOException,
                                                                                                  xBaseJException {
        boolean madebigger;
        long startPos;
        int pos;
        ...
        if (value.length() == 0) {
            byte breturn[] = {
                BYTEZERO, BYTEZERO, BYTEZERO, BYTEZERO, BYTEZERO, BYTEZERO, BYTEZERO, BYTEZERO, BYTEZERO, BYTEZERO };
            return breturn;
        }
